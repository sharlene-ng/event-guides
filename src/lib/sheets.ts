...
async function callGet(
  ...
  const res = await fetch(`${API_URL}?${params.toString()}`, {
    method: "GET",
    redirect: "follow",
    // Cached for fast loads. Detail pages purge their own cache after an edit
    // via revalidatePath(`/events/${id}`), so refreshes stay fresh.
...
export async function getEvent(id: string): Promise<SOPEvent | null> {
  // Cached for fast loads; edits purge it via revalidatePath(`/events/${id}`)
  // so a checklist tick / approval shows immediately on refresh.
